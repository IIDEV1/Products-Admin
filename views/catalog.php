...
<div class="mb-16">
    <h1 class="text-5xl font-extrabold tracking-tight text-slate-900 mb-4"><?= __('nav_catalog') ?></h1>
    <div class="w-20 h-1.5 bg-indigo-600 rounded-full"></div>
</div>

<div class="luxury-card rounded-2xl p-8 mb-12 flex flex-wrap gap-6 items-center bg-white shadow-sm border border-slate-200">
    <div class="flex-grow max-w-lg relative">
        <input type="text" id="catalogSearch" placeholder="<?= __('search_placeholder') ?>" 
               class="w-full bg-slate-50 border border-slate-200 rounded-xl px-6 py-4 text-sm font-medium focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600 outline-none transition-all">
    </div>
    <div class="flex gap-4">
        <input type="number" id="priceMin" placeholder="МИН" 
               class="w-24 bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600">
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8" id="productGrid">
    <?php foreach ($products as $i => $product): ?>
        <div class="luxury-card rounded-2xl flex flex-col bg-white overflow-hidden group border border-slate-200" 
            ...
            <a href="/?page=product&id=<?= $product['id'] ?>" class="block aspect-square overflow-hidden bg-slate-100">
            ...
            <div class="p-6 flex flex-col flex-grow">
                <a href="/?page=product&id=<?= $product['id'] ?>" class="block mb-2">
                    <h3 class="text-lg font-bold text-slate-900 group-hover:text-indigo-600 transition-colors tracking-tight">
                ...
                <div class="mt-auto flex items-center justify-between pt-6 border-t border-slate-100">
                    <div class="flex flex-col">
                        ...
                        <span class="text-xl font-extrabold text-slate-900"><?= number_format($product['price'], 0, '.', ' ') ?> <span class="text-indigo-600 text-sm italic"><?= __('currency') ?></span></span>
                    </div>
                    <a href="actions/cart.php?action=add&id=<?= $product['id'] ?>" 
                            class="bg-slate-900 text-white p-3 rounded-xl hover:bg-indigo-600 transition-all active:scale-95 shadow-md">
